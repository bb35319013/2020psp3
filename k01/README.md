...
## ソースコードの説明
68.resultが-1を返すようにしておく
69.posの初期化、posがデータの数より小さければ以下を繰り返し、以上なら終了、繰り返すときにposに1加える
70.検索する市町村のidがデータの中にあったら以下を実行
71.resultにposを代入
72.ループを抜ける
75.resultを返す

81.posの初期化、resultが-1を返すようにしておく
83.リストにデータがあれば以下を繰り返し、なければ終了
84.比較対象をリストの真ん中に設定する
85.検索する市町村のidが比較対象と同じだったら以下を実行
86.resultにposを代入
87.ループを抜ける
89.検索する市町村のidが比較対象より大きかったら以下を実行
90.leftにpos+1を代入
91.
92.検索する市町村のidが比較対象より小さかったら以下を実行
93.rightにpos-1を代入
96.resultを返す
## 出力結果
